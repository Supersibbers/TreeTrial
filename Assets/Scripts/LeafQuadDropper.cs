...
public class LeafQuadDropper : MonoBehaviour {
	...
	public float leafRange = 100;
	public float pileRange = 100;
	...
	void Start () {
		...
		foreach (MyTree treeScript in treeArray) {
			...
			for (int i = 0; i < Random.Range (5, 15); i++) {
				Vector3 position = tree.transform.position + new Vector3 (Random.Range (-leafRange, leafRange), 0.01f, Random.Range (-leafRange, leafRange));
				...
				fallenLeaf.GetComponent<Renderer> ().material.color = autumnController.materials [treeScript.leafMaterialX, treeScript.leafMaterialY].color;
			}
			...
			for (int i = 0; i < Random.Range (0, 5); i++) {
				...
				Quaternion rotation = Quaternion.Euler (-90, Random.Range (0, 359), 0);
				GameObject fallenLeaf = Instantiate (leaves [0], position, rotation, this.transform) as GameObject;
				fallenLeaf.GetComponent<Renderer> ().material.color = autumnController.materials [treeScript.leafMaterialX - (2 * treeScript.leafMaterialSpread), treeScript.leafMaterialY - (2 * treeScript.leafMaterialSpread)].color;
			}

			for (int i = 0; i < Random.Range (0, 3); i++) {
				Vector3 position = tree.transform.position + new Vector3 (Random.Range (-pileRange, pileRange), 0, Random.Range (-pileRange, pileRange));
				Quaternion rotation = Quaternion.Euler (-90, Random.Range (0, 359), 0);
